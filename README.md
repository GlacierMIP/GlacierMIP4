...
- **Ice thickness**: GlacierMIP4 uses the data by [Maffezzoli et al. (2026)](https://arxiv.org/abs/2512.11685). Gridded ice thickness is provided at the [individual-glacier level](https://zenodo.org/records/17724512), [glacier complex level](https://zenodo.org/records/21220985), and [per UTM zone](https://zenodo.org/records/20463551). **Important (🟥 added 2026/07/06):** because of their spatial resolution and glacier mask, the per-glacier gridded Iceboost v2 products contain more ice than the glacier's estimated volume (see [GH issue](https://github.com/nmaffe/iceboost/issues/21)). The correct glacier volume is available as an attribute of the individual GeoTIFF files, or aggregated as global data tables: **RGI70G** [csv](https://cluster.klima.uni-bremen.de/~oggm/ice_thickness/iceboost_v2/compiled_gmip4/iceboost_compiled_rgi70G_v20260705.csv), [parquet](https://cluster.klima.uni-bremen.de/~oggm/ice_thickness/iceboost_v2/compiled_gmip4/iceboost_compiled_rgi70G_v20260705.parquet), **RGI70C** [csv](https://cluster.klima.uni-bremen.de/~oggm/ice_thickness/iceboost_v2/compiled_gmip4/iceboostv2_compiled_rgi70C_v20260705.csv), [parquet](https://cluster.klima.uni-bremen.de/~oggm/ice_thickness/iceboost_v2/compiled_gmip4/iceboostv2_compiled_rgi70C_v20260705.parquet). If you need the gridded thicknesses, we recommend reprojecting onto your domain, cropping to the glacier mask, and rescaling the result to match the volume attribute. **Table 1** provides ice volumes aggregated by RGI region.
- **Outline dates (🟥 added 2026/07/06)**: RGI outline dates vary regionally and even within regions (e.g. much of Antarctica pre-dates 2000 by years to decades; High Mountain Asia spans the 1990s to ~2010). GlacierMIP4's convention is to treat the IceBoostv2 volumes as valid at the RGI outline date. Modelers may either adopt these volumes directly at the RGI date, or as valid at the year 2000. Both are acceptable, but **please document which approach you use** so this can be accounted for when assessing model performance. In either case, the resulting **regional ice volumes** must agree with the reference values in **Table 1** within **±10%**. *(Note: how this check applies to regions whose mean RGI date is far from 2000, e.g. Antarctica, is still being finalized — update to follow.)*

**Table 1.** Regional glacier area and volume. Areas are from RGI Consortium (2023). Volumes are from Maffezzoli et al. (2026).

| Region | Full name | Area (km²) | Volume (10³ km³) |
|:-------|:----------------------------------|------------:|------------:|
| 01 | Alaska | 86708 | 16.9 |
| 02 | Western Canada and USA | 14521 | 1.5 |
| 03 | Arctic Canada North | 105370 | 24.3 |
| 04 | Arctic Canada South | 40538 | 7.1 |
| 05 | Greenland Periphery | 90482 | 13.2 |
| 06 | Iceland | 11060 | 4.6 |
| 07 | Svalbard and Jan Mayen | 33959 | 6.7 |
| 08 | Scandinavia | 2948 | 0.35 |
| 09 | Russian Arctic | 51595 | 12.8 |
| 10 | North Asia | 2643 | 0.19 |
| 11 | Central Europe | 2124 | 0.11 |
| 12 | Caucasus and Middle East | 1407 | 0.08 |
| 13 | Central Asia | 50344 | 3.8 |
| 14 | South Asia West | 33075 | 3.8 |
| 15 | South Asia East | 16049 | 1.0 |
| 16 | Low Latitudes | 1929 | 0.09 |
| 17 | Southern Andes | 27674 | 6.7 |
| 18 | New Zealand | 886 | 0.08 |
| 19 | Subantarctic and Antarctic Islands | 133432 | 45.7 |
| Global |  | 706744 | 149 |
...
- **Projections**: All models must force their models with bias-corrected CMIP6 GCM data from the eight GCMs listed in **Table 2** and four SSP scenarios (SSP1-2.6, SSP3-7.0, SSP5-8.5 and the overshoot scenario SSP5-3.4OS). Scenarios and GCMs were selected to align as much as possible with the protocol used by [ISMIP](https://www.ismip.org/). We include the two GCMs used by ISMIP (CESM2-WACCM and MRI-ESM2-0) and selected additional GCMs based on [Snyder et al. (2024)](https://doi.org/10.5194/esd-15-1301-2024). We selected all but one of the GCMs listed in their Table 3, spanning a wide range of climate sensitivities. We also added the NorESM2-MM, which, despite structural dependency, displays significantly lower equilibrium climate sensitivity than CESM2-WACCM (Table 2).

We **bias-corrected** monthly temperature and precipitation of all GCM projections to the ERA5 reanalysis, and the data is available to all participants [here](https://cluster.klima.uni-bremen.de/~oggm/cmip6/era5_biascorr/). The link contains two folder trees: `raw/` stores the original bias-corrected NetCDF files without compression and with the full production time coverage (total 764G), while `compressed/` contains the recommended user-facing files with lossless compression, monthly timestamps standardized to the first day of each month, and data from 2000 onward (total 287G). The bias-correction follows the quantile delta mapping method (e.g., Cannon et al. (2015); Lader et al. (2017)) which preserves trends in future climate while removing systematic bias relative to historical observations. Specifically, the change across quantiles in each variable is calculated relative to the 1980-2019 reference period in 30-year windows and this change factor is applied to the distribution of ERA-5 reference data. If you need additional variables or use a different time resolution you will need to prepare the data and bias-correct the GCM data yourself.
...
- Glacier-wide specific geodetic mass balances derived from an RGI7 updated version of **Hugonnet et al. (2021)** available [here](https://drive.google.com/drive/folders/1jXEQeYN8Swz-B2CYFxpFUlSWpgdvm7o0?usp=sharing). **This is unpublished data reserved for GlacierMIP4 use only.** For use outside GlacierMIP4, contact [rhugonnet@alaska.edu](rhugonnet@alaska.edu).
